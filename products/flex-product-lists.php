...
<div class="w-full flex items-center  gap-4 my-4">
    ...
    <?php if (mysqli_num_rows($result) > 0): ?>
        ...
        <button 
            class="hidden add-to-cart-btn bg-[#C2185B] text-white rounded-[8px] text-[15px] md:text-[16px] font-Onest font-medium cursor-pointer p-2 w-[90%] absolute bottom-0 left-[5%]"
...
<div class="actionstab absolute w-[fit-content] right-2 top-4">
                        <div class="flex flex-col gap-2 md:gap-4">
                            <i class="cursor-pointer add-to-favourite text-[30px] md:text-[35px] leading-none drop-shadow-md <?php echo isset($favorites[$product['product_id']]) ? 'fa-solid fa-heart favorite-active text-[#C2185B]' : 'fa-regular fa-heart text-white'; ?>" 
             data-product-id="<?php echo $product['product_id']; ?>"></i>
       
                            
<i class="fa-solid <?php echo array_key_exists($product['product_id'], $cart_items) ? 'fa-circle-check text-[#C2185B]' : 'fa-cart-plus text-white'; ?> cursor-pointer cart-toggle-icon text-[30px] md:text-[35px] leading-none drop-shadow-md <?php echo array_key_exists($product['product_id'], $cart_items) ? 'in-cart' : ''; ?>" 
     data-product-id="<?php echo $product['product_id']; ?>"
     data-cart-id="<?php echo array_key_exists($product['product_id'], $cart_items) ? $cart_items[$product['product_id']] : ''; ?>"
></i>
                            </div>
