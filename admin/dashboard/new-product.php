...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'create_product') {
    ...
    $insert_product = "INSERT INTO products (product_name, category_id, brand_id, sku, colors, details, sizes_details, warranty, care, is_featured) 
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($con, $insert_product);
    mysqli_stmt_bind_param($stmt, "siissssssi", $product_name, $category_id, $brand_id, $sku, $colors, $details, $sizes_details, $warranty, $care, $is_featured);
